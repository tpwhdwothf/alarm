...
#### 메모리 스파이크 시 어떤 프로세스가 썼는지 확인하는 방법

**다음에 메모리가 튈 때**  
`tail -50 ~/ssh-debug.log` 에서 끊긴 시각에 가까운 줄 **바로 아래** `top_mem:` 이 있으면, 그 시각에 메모리를 많이 쓰던 프로세스 목록입니다.

- 예: `top_mem: node 42% 180MB, ocarun 8% 40MB, ...`  
  → 그때는 **node**(봇/워커)가 42%로 가장 많이 쓰고 있었음.

**이미 재부팅된 뒤** (직전 부팅에서 OOM으로 프로세스가 죽었는지 확인):

```bash
sudo journalctl -b -1 --no-pager | grep -i "killed process\|out of memory"
```

- 결과에 `Killed process 1234 (node)` 처럼 나오면, 직전 부팅에서는 **그 프로세스**가 메모리 부족으로 종료된 것입니다.
