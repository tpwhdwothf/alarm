...
const serverHealthMonitor_1 = require("../lib/serverHealthMonitor");
dotenv.config();
...
async function findTargetByInput(userId, input, selectColumns) {
...
bot.onText(/^\/setgroup(?:\s+(.+))?$/, async (msg, match) => {
    ...
    try {
        const { error: groupError } = await supabaseClient_1.supabase
            .from("alert_groups")
            .upsert({
            created_by: userId,
...
bot.onText(/^\/(add|등록) (.+)$/, async (msg, match) => {
    ...
    if (pickType !== "무료픽" && pickType !== "VIP픽") {
        bot.sendMessage(msg.chat.id, "두 번째 인자는 '무료픽' 또는 'VIP픽' 이어야 합니다.\n예) /등록 코길동 무료픽 현대차 005380 660000 675000");
        return;
    }
    if (!nameInput || !rawSymbol) {
        bot.sendMessage(msg.chat.id, "종목명과 종목코드를 정확히 입력해주세요.\n예) /등록 코길동 무료픽 현대차 005380 660000 675000");
        return;
    }
    ...
    if (tps.length === 0) {
        bot.sendMessage(msg.chat.id, "목표가는 숫자로 입력해야 합니다.\n예) /등록 코길동 무료픽 현대차 005380 660000 675000");
        return;
    }
    const { data: userSettings, error: userError } = await supabaseClient_1.supabase
        .from("user_settings")
        .select("*")
    ...
    if (!userSettings || !userSettings.default_group_chat_id) {
        bot.sendMessage(msg.chat.id, "먼저 알림을 받을 그룹 채팅방에서 /setgroup 을 한 번 실행해주세요.");
        return;
    }
    const market = detectMarket(rawSymbol);
    const upperSymbol = rawSymbol.toUpperCase();
    const { error } = await supabaseClient_1.supabase.from("targets").upsert({
        created_by: userId,
        symbol: upperSymbol,
    ...
    if (error) {
        ...
        bot.sendMessage(msg.chat.id, `목표가를 저장하는 중 오류가 발생했어요.${hint}\n오류: ${error.message}`);
        return;
    }
...
bot.onText(/^\/(edit|수정) (.+)$/, async (msg, match) => {
    ...
    if (!symbol || tpStrings.length === 0) {
        bot.sendMessage(msg.chat.id, "사용법: /edit 종목 tp1 tp2 ... 또는 /수정 종목 tp1 tp2 ...\n예) /edit AAPL 185 195 205");
        return;
    }
    const tps = tpStrings.map(Number).filter((n) => !Number.isNaN(n));
    if (tps.length === 0) {
        bot.sendMessage(msg.chat.id, "목표가는 숫자로 입력해야 합니다. 예) /edit AAPL 185 195 205 또는 /수정 AAPL 185 195 205");
        return;
    }
    ...
    if (!existing) {
        bot.sendMessage(msg.chat.id, `해당 종목이 없습니다: ${upperSymbol}\n먼저 /add 명령으로 종목을 등록한 뒤 /edit 을 사용해주세요.`);
        return;
    }
    ...
    bot.sendMessage(msg.chat.id, `종목 ${upperSymbol}의 목표가를 수정했습니다.\n기존: ${oldTpsText}\n변경: ${newTpsText}\nnext_level 을 1로 초기화했습니다.`);
});
bot.onText(/^\/(append|추가) (.+)$/, async (msg, match) => {
    ...
    const tpStrings = parts.slice(1);
    if (!symbol || tpStrings.length === 0) {
        bot.sendMessage(msg.chat.id, "사용법: /append 종목 tpN tpN+1 ... 또는 /추가 종목 tpN tpN+1 ...\n예) /append AAPL 220 230");
        return;
    }
    const newTps = tpStrings.map(Number).filter((n) => !Number.isNaN(n));
    if (newTps.length === 0) {
        bot.sendMessage(msg.chat.id, "목표가는 숫자로 입력해야 합니다. 예) /append AAPL 220 230 또는 /추가 AAPL 220 230");
        return;
    }
    const upperSymbol = symbol.toUpperCase();
    ...
    const baseTps = Array.isArray(existing.tps)
        ? existing.tps.map((v) => Number(v)).filter((n) => !Number.isNaN(n))
        : [];
    ...
    const statusMessage = existing.status === "COMPLETED" && newStatus === "ACTIVE"
...
bot.onText(/^\/(list|목록)$/, async (msg) => {
    ...
    try {
        ...
        const { data, error } = await client
            ...
            .order("symbol");
        if (error) {
            console.error("[목록] Supabase 오류:", (_a = error === null || error === void 0 ? void 0 : error.message) !== null && _a !== void 0 ? _a : error);
...
bot.onText(/^\/(status|상태) (.+)$/, async (msg, match) => {
    ...
    if (!symbolInput) {
        bot.sendMessage(msg.chat.id, "사용법: /status 종목 또는 /상태 종목\n예) /status AAPL");
        return;
    }
    ...
    const nextTp = Array.isArray(data.tps) && nextLevel >= 1 && nextLevel <= tps.length
...
bot.onText(/^\/(setlevel|목표) (.+)$/, async (msg, match) => {
    ...
    const tps = Array.isArray(data.tps)
        ? data.tps.map((v) => Number(v)).filter((n) => !Number.isNaN(n))
        : [];
    if (tps.length === 0) {
    ...
    const nextTp = nextIdx >= 0 && nextIdx < tps.length
        ? String(tps[nextIdx])
        : "모든 목표가 도달 또는 없음";
    bot.sendMessage(msg.chat.id, [
...
bot.onText(/^\/(close|종료) (.+)$/, async (msg, match) => {
    ...
    const symbolInput = match && match[2] ? match[2].trim() : "";
    if (!symbolInput) {
        bot.sendMessage(msg.chat.id, "사용법: /close 종목 또는 /종료 종목\n예) /close AAPL");
        return;
    }
    const symbol = symbolInput.toUpperCase();
    ...
    if (!data) {
        bot.sendMessage(msg.chat.id, `종목이 없거나 이미 CLOSED 상태입니다: ${symbol}\n먼저 /add 로 등록했는지 확인해주세요.`);
        return;
    }
    const { error: updateError } = await supabaseClient_1.supabase
        .from("targets")
...
bot.onText(/^\/(open|재개) (.+)$/, async (msg, match) => {
    ...
    const symbolInput = match && match[2] ? match[2].trim() : "";
    if (!symbolInput) {
        bot.sendMessage(msg.chat.id, "사용법: /open 종목 또는 /재개 종목\n예) /open AAPL");
        return;
    }
    const symbol = symbolInput.toUpperCase();
    ...
    if (!data || data.status !== "CLOSED") {
        bot.sendMessage(msg.chat.id, `CLOSED 상태인 종목이 없거나 찾을 수 없습니다: ${symbol}\n먼저 /close 로 종료한 종목인지 확인해주세요.`);
        return;
    }
    const { data: reopened, error: updateError } = await supabaseClient_1.supabase
        .from("targets")
    ...
    bot.sendMessage(msg.chat.id, `종목 ${symbol} 을(를) 다시 ACTIVE 상태로 변경했습니다.\nnext_level=${data.next_level} 부터 알림이 재개됩니다.`);
});
bot.onText(/^\/(delete|삭제) (.+)$/, async (msg, match) => {
    ...
    const symbolInput = match && match[2] ? match[2].trim() : "";
    if (!symbolInput) {
        bot.sendMessage(msg.chat.id, "사용법: /delete 종목 또는 /삭제 종목\n예) /delete AAPL");
        return;
    }
    const symbol = symbolInput.toUpperCase();
    ...
    if (!data) {
        bot.sendMessage(msg.chat.id, `해당 종목을 찾을 수 없습니다: ${symbol}\n먼저 /add 로 등록했는지 확인해주세요.`);
        return;
    }
    ...
    bot.sendMessage(msg.chat.id, `종목 ${symbol} 을(를) 목록에서 삭제했습니다.\n더 이상 이 종목에 대해서는 알림이 발생하지 않습니다.`);
});
bot.onText(/^\/health$/, async (msg) => {
    ...
    if (!supabaseClient_1.supabase) {
        ...
        try {
            const { error } = await supabaseClient_1.supabase.from("targets").select("id").limit(1);
            if (error) {
            ...
            lines.push(`- Supabase: ⚠️ 예외 발생 (${(e === null || e === void 0 ? void 0 : e.message) || String(e)})`);
        }
    }
    // 간단 버전 정보
...
bot.on("new_chat_members", async (msg) => {
    ...
    if (!supabaseClient_1.supabase || !msg.new_chat_members || msg.new_chat_members.length === 0) {
        return;
    }
    const chatId = String(msg.chat.id);
...
const DISABLE_HEALTH_MONITOR = process.env.DISABLE_HEALTH_MONITOR === "true" || process.env.DISABLE_HEALTH_MONITOR === "1";
